...
from services.mainflow.app.config.constants import ALLOWED_ANALYSIS_TYPES, ALLOWED_ASSET_TYPES
...
def validate_asset_type(value: str) -> str:
    asset_type = ""
    for akey, avalue in ALLOWED_ASSET_TYPES.items():
        if avalue == value:
            asset_type = akey
            break
    print(f"Validated asset type: {asset_type}")
    return asset_type
