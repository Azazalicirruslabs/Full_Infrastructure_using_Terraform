...
class FairnessAnalysisRequest(BaseModel):
    """
    Complete request schema for fairness analysis with auto-detection support.
    ...
    Attributes:
        ...
        target_column: Name of the target/label column (optional - will auto-detect if not provided)
        sensitive_feature: Name of the sensitive/protected attribute column (optional - will auto-detect if not provided)
    ...
    target_column: Optional[str] = Field(
        None, description="Target/label column name (optional - will auto-detect as last column if not provided)"
    )
    sensitive_feature: Optional[str] = Field(
        None,
        description="Sensitive/protected attribute column name (optional - will auto-detect using HSIC Lasso if not provided)",
    )
...
class FairnessAnalysisResponse(BaseModel):
    """
    Complete fairness analysis response with auto-detection metadata.

    Contains calculated metrics, group performance comparison, LLM analysis,
    auto-detection details, and overall assessment.
    """
    ...
    auto_detection: Dict[str, Any] = Field(
        default_factory=dict, description="Auto-detection metadata (target column and sensitive feature)"
    )
