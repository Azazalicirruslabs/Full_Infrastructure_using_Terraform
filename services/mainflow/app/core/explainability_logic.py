...
class ExplainabilityService:
    ...
    def load_model_and_datasets(
        ...
        Args:
            ...
            target_column: Name of the target column (optional, defaults to last column if not provided)
        ...
        try:
            ...
            # Auto-detect target column if not provided (use last column)
    ...
    def compute_explainability_analysis(
        self, compute_feature_importance: bool = True, method: str = "shap"
    ) -> Dict[str, Any]:
        """
        ...
        Returns:
            Dict with computation metadata, LLM analysis, and feature importance details
        """
        self._ensure_loaded()
        ...
        # Generate LLM analysis (only for feature importance, not performance metrics)
        llm_analysis = {}
        try:
            llm_analyzer = LLMBasedExplainabilityAnalysis()
            llm_analysis = llm_analyzer.analyze_explainability_results(
                feature_importance=feature_importance, model_type=model_type
            )
        ...
        # Build response in the requested format with top-level fields
        result = {
            "model_type": model_type.title(),
            "computation_method": feature_importance.get("computation_method", method),
            "computed_at": feature_importance.get("computed_at", pd.Timestamp.utcnow().isoformat()),
            "llm_analysis": llm_analysis,
            "shap_available": feature_importance.get("shap_available", False),
            "target_column": self.target_name,
            "total_features": feature_importance.get("total_features", len(self.feature_names)),
            "positive_impact_count": feature_importance.get("positive_impact_count", 0),
            "negative_impact_count": feature_importance.get("negative_impact_count", 0),
    ...
    def _load_dataset(self, path: str) -> pd.DataFrame:
        ...
        if path.startswith(("http://", "https://")):
            ...
            try:
                ...
                resp = make_safe_request(validated_url, timeout=30)
...
class LLMBasedExplainabilityAnalysis:
    ...
    def analyze_explainability_results(self, feature_importance: Dict[str, Any], model_type: str) -> Dict[str, str]:
        """
        Generate LLM analysis for feature importance explainability results.

        Args:
            feature_importance: Feature importance results (SHAP or other methods)
        ...
        summary = self._build_explainability_summary(feature_importance, model_type)

        prompt = f"""You are an expert in machine learning model explainability and interpretability. Analyze the following feature importance results:

{summary}

Provide a concise analysis in THREE sections, paying special attention to the computation method used:

1. **What This Means** (2-3 sentences)
   Explain what the feature importance values reveal about the model's decision-making process, considering the specific computation method used (SHAP, Gain, or Permutation). Which features are most influential, what are their impact directions, and how does the chosen method influence the interpretation?

2. **Why It Matters** (2-3 sentences)
   Explain the business implications of these feature importances. What should stakeholders understand about which factors drive predictions? Consider the reliability and interpretability characteristics of the computation method used.

3. **Risk Signal** (1-2 sentences)
   Identify any concerns in the feature importance distribution, such as over-reliance on specific features, unexpected feature impacts, or limitations of the computation method. Highlight potential data quality issues or methodology considerations.

Use clear, non-technical language suitable for business stakeholders. Be direct and specific about both the findings and the method used."""

        analysis_text = self._invoke_claude(prompt)

        # Parse the response into structured sections
        return self._parse_analysis(analysis_text, feature_importance)

    def _build_explainability_summary(self, feature_importance: Dict[str, Any], model_type: str) -> str:
        """Build a text summary of feature importance for LLM analysis."""
        summary = f"=== Model Type: {model_type.title()} ===\n\n"

        # Feature Importance
        computation_method = feature_importance.get("computation_method", "unknown").upper()
        summary += f"=== Feature Importance ({computation_method}) ===\n\n"
    ...
    def _parse_analysis(self, analysis_text: str, feature_importance: Dict[str, Any]) -> Dict[str, str]:
        ...
        if not result["what_this_means"]:
            result["what_this_means"] = (
                f"Using {computation_method} analysis, the feature importance reveals {total_features} features with "
                f"{positive_count} showing positive impact and {negative_count} showing negative impact on predictions."
            )

        if not result["why_it_matters"]:
            method_context = ""
            if computation_method == "SHAP":
                method_context = "SHAP provides game-theory based feature attributions, offering reliable insights into feature contributions. "
            elif computation_method == "GAIN":
                method_context = "Gain-based importance shows which features are most frequently used in model splits. "
            elif computation_method == "PERMUTATION":
                method_context = (
                    "Permutation importance measures the impact of shuffling each feature on model performance. "
                )

            result["why_it_matters"] = (
                f"{method_context}"
                f"Understanding feature importance helps identify which factors drive model predictions, "
                f"enabling better decision-making and revealing potential biases or unexpected dependencies."
            )

        if not result["risk_signal"]:
            result["risk_signal"] = (
                f"Feature importance distribution using {computation_method} appears reasonable. "
                f"Continue monitoring for unexpected changes in feature impacts over time."
            )
