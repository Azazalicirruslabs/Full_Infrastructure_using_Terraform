...
# Import sensitivity detector for auto-detection feature
from services.mainflow.app.utils.sensitivity_detector import SensitiveFeatureDetector
...
class FairnessSummaryCalculator:
    ...
    # ===================================================================================
    # AUTO-DETECTION METHODS (New Implementation)
    # ===================================================================================

    def auto_detect_target_column(self, df: pd.DataFrame) -> str:
        """
        Auto-detect target column by assuming it's the last column in the dataset.

        Args:
            df: DataFrame from which to detect target column

        Returns:
            Name of the detected target column (last column)

        Raises:
            ValueError: If DataFrame is empty
        """
        if df.empty or len(df.columns) == 0:
            raise ValueError("Cannot auto-detect target column: DataFrame is empty")

        target_col = df.columns[-1]
        logger.info(f"Auto-detected target column: '{target_col}' (last column)")
        return target_col

    def auto_detect_sensitive_feature(
        self, df: pd.DataFrame, target_col: str, n_top_features: int = 10
    ) -> Dict[str, Any]:
        """
        Auto-detect sensitive features using HSIC Lasso-based statistical dependency analysis.

        Args:
            df: DataFrame containing all features including target
            target_col: Name of the target column
            n_top_features: Number of top features to consider in detection

        Returns:
            Dictionary containing:
                - top_sensitive_feature: The highest-ranked sensitive feature (str)
                - all_sensitive_features: List of all detected sensitive features with scores
                - detection_method: Method used for detection (str)
                - threshold_value: Sensitivity threshold used (float)

        Raises:
            ValueError: If detection fails or no features available
        """
        logger.info("Starting automatic sensitive feature detection using HSIC Lasso...")
        ...
        if X.empty or len(X.columns) == 0:
            raise ValueError("No features available for sensitivity detection (only target column exists)")

        logger.info(f"Analyzing {len(X.columns)} features for sensitivity to target '{target_col}'")

        try:
            # Initialize detector with default threshold (75th percentile)
            detector = SensitiveFeatureDetector(threshold_percentile=75)

            # Run detection
            sensitive_features_list, feature_scores, threshold = detector.detect_sensitive_features(
                X, y, n_features_to_select=n_top_features
            )

            # Get detailed scores
            detailed_scores = detector.get_feature_sensitivity_scores()

            # Extract top-ranked feature
            if detailed_scores and len(detailed_scores) > 0:
                top_feature = detailed_scores[0]["feature"]
                top_score = detailed_scores[0]["sensitivity_score"]
                logger.info(
                    f"Top-ranked sensitive feature: '{top_feature}' (score: {top_score:.4f}, "
                    f"found {len(sensitive_features_list)} total sensitive features)"
                )
            else:
                raise ValueError("No sensitive features detected")

            return {
                "top_sensitive_feature": top_feature,
                "all_sensitive_features": detailed_scores,
                "detection_method": "HSIC Lasso",
                "threshold_value": threshold,
                "total_features_analyzed": len(X.columns),
                "sensitive_features_count": len(sensitive_features_list),
            }

        except Exception as e:
            logger.error(f"Sensitive feature detection failed: {e}")
            raise ValueError(f"Could not auto-detect sensitive feature: {str(e)}")
    ...
    def process_fairness_request(
        ...
        metric_selection: Dict[str, bool] = None,
        ...
        """
        Main entry point for fairness analysis request with auto-detection support.

        This orchestrates the complete analysis workflow:
        1. Validate inputs
        2. Download and preprocess datasets
        3. Auto-detect target_column and/or sensitive_feature if not provided
        4. Load model and generate predictions (if needed)
        5. Calculate selected metrics
        6. Format complete API response

        Args:
            reference_url: S3 URL to reference dataset
            target_column: Name of target column (Optional - will auto-detect if not provided)
            sensitive_feature: Name of sensitive feature (Optional - will auto-detect if not provided)
        ...
        Returns:
            Complete API response dictionary including detection metadata
        ...
        # Track auto-detection metadata
        auto_detection_metadata = {
            "target_column_auto_detected": False,
            "sensitive_feature_auto_detected": False,
            "detection_details": {},
        }
        ...
        try:
            ...
            # 3.5. AUTO-DETECTION: Target column and sensitive feature
            logger.info("Step 3.5: Auto-detecting target column and/or sensitive feature if needed...")

            # Auto-detect target column if not provided
            if target_column is None or target_column == "":
                logger.info("Target column not provided, auto-detecting...")
                target_column = self.auto_detect_target_column(df_reference)
                auto_detection_metadata["target_column_auto_detected"] = True
                auto_detection_metadata["detection_details"]["target_column"] = {
                    "detected_value": target_column,
                    "method": "last_column",
                }
            else:
                logger.info(f"Using user-provided target column: '{target_column}'")

            # Auto-detect sensitive feature if not provided (requires target column)
            if sensitive_feature is None or sensitive_feature == "":
                logger.info("Sensitive feature not provided, auto-detecting...")
                detection_result = self.auto_detect_sensitive_feature(df_reference, target_column, n_top_features=10)
                sensitive_feature = detection_result["top_sensitive_feature"]
                auto_detection_metadata["sensitive_feature_auto_detected"] = True
                auto_detection_metadata["detection_details"]["sensitive_feature"] = {
                    "detected_value": sensitive_feature,
                    "method": detection_result["detection_method"],
                    "all_detected_features": detection_result["all_sensitive_features"],
                    "threshold_value": detection_result["threshold_value"],
                    "total_features_analyzed": detection_result["total_features_analyzed"],
                    "sensitive_features_count": detection_result["sensitive_features_count"],
                }
            else:
                logger.info(f"Using user-provided sensitive feature: '{sensitive_feature}'")
            ...
            response = {
                ...
                "data": {
                    **metrics_result,  # key_metrics, disparate_impact_ratio, detailed_breakdown, group_performance
                    ...
                    "auto_detection": auto_detection_metadata,
                    "metadata": {
