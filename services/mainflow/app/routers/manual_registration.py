...
from services.mainflow.app.utils.helper_function import check_analysis, validate_asset_type
...
def manual_registration(
    ...
    try:
        ...
        asset_type = validate_asset_type(payload.asset_type)
        if not asset_type:
            logger.error(f"Invalid asset type provided | user_id={user_id}, asset_type={payload.asset_type}")
            raise HTTPException(status_code=400, detail="Invalid asset type provided.")

        analysis = check_analysis(payload.evaluation_type)
        if not analysis:
        ...
        existing = (
            ...
            .filter_by(
                tenant_id=tenant_id,
                analysis_type=payload.evaluation_type,
        ...
        logger.info(
            ...
            f"analysis_type={payload.evaluation_type}, "
