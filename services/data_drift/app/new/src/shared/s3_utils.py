...
def load_s3_csv(url: str) -> pd.DataFrame:
    ...
    try:
        ...
        # ✅ Extract file extension safely (ignore query params)
        ...
        if file_ext == ".csv":
            ...
            raise HTTPException(
                status_code=400, detail=f"Unsupported file type: {file_ext}. Only CSV and Parquet are allowed."
            )

        if df.empty:
            raise HTTPException(status_code=400, detail="Uploaded file is empty")

        logger.info(f"Loaded file from {url}: shape={df.shape}")
        return df
        ...
        logger.error(f"Unexpected file load error: {e}")
